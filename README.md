
# 🚀 About Me
👋 Hi! I'm **Abdul-Azeez**, an **AI Systems Engineer**.  

I build AI systems that are:  
- ⚡ **Scalable & Efficient** – Optimized ML models for real-world use  
- 🤖 **LLM-Powered** – Intelligent systems that reason and learn  
- 💡 **Practical** – Tools that solve everyday problems
---

## 🧠 Current Focus
-  💰 SaaS
-  🧠 Intelligent Multi-Agent Systems
-  🔍 LLM Evaluation & Observability
-  📚 AI Research Copilots
-  ⚙️ Scalable AI Backend Architecture

#### 🚀 Projects I Have Built

| Project | Description | Tech | Link |
|---------|-------------|------|------|
| 🎯 **ACHIEVIT** | Adaptive LLM Academic Planning System | Python, FastAPI, LLM | [View App](https:captai.streamlit.app) |
| 🎭 **CaptAI** | ML-powered sentiment analysis | Python, PyTorch, NLP | [View App](https://github.com/Abdul-WriteCodes/CaptAI) |
| 💳 **FinRisk-ML** | Transaction risk analysis system | Python, scikit-learn, ML | [View App](https://github.com/Abdul-WriteCodes/FinRisk-ML) |
| 🩺 **Chron-GPT** | LLM system for diabetes risk analysis | Python, LLM, Healthcare AI | [View App](https://github.com/Abdul-WriteCodes/Chron-GPT) |


---
#  🚀Projects I have Built
-  🎯 **ACHIEVIT** – An Adaptive LLM Academic Planning System
-  🎭 **CaptAI** – A supervised machine learning models powered system for sentiment analysis
-  💳 **FinRisk-ML**- A supervised machine learning model-powered system for analysing transaction risk
-  🩺 **Chron-GPT**- A modular LLM-powered intelligent system that analyse diabetes risk and provide realtime support


---

# 🛠️ Tech Stack
<p align="left">
  - Languages: <img src="https://skillicons.dev/icons?i=python,typescript,mysql,mongodb,postgres,go" />
</p>

<p align="left">
 - Backend: <img src="https://skillicons.dev/icons?i=fastapi,nodejs,docker" />
</p>

<p align="left">
  - Frontend: <img src="https://skillicons.dev/icons?i=nextjs,react,tailwind" />
</p>

<p align="left">
  - AI/ML Frameworks: <img src="https://skillicons.dev/icons?i=pytorch,scikitlearn,tensorflow" />
</p>

<p align="left">
 - Deployment and Cloud: <img src="https://skillicons.dev/icons?i=docker,vercel,aws" />
</p>

---
## Stats

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=Abdul-WriteCodes&theme=radical)


# 📫 Connect With Me

<p align="left">
  <a href="https://www.linkedin.com/in/abdul-azeez-olohunjuwon-anafi-baa64a199/">
    <img src="https://skillicons.dev/icons?i=linkedin" />
  </a>
  <a href="mailto:Abdulwrite77@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" />
  </a>

  <a href="https://wa.me/2348096506034" target="_blank">
    <img src="https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg" alt="WhatsApp" width="30" />
  </a>
</p>

